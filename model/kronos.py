...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy() # Shape: (B, sample_count, pred_len, feat)
        return preds
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose):
        ...
        # preds shape is (B, sample_count, total_seq_len, feat)
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        # preds: (1, sample_count, pred_len, feat)
        preds = preds.squeeze(0) # (sample_count, pred_len, feat)
        
        preds = preds * (x_std + 1e-5) + x_mean
        
        if sample_count > 1:
            # We have multiple paths. Calculate mean, min, max
            preds_mean = np.mean(preds, axis=0)
            preds_min = np.percentile(preds, 10, axis=0) # 10th percentile for Best/Worst mapping
            preds_max = np.percentile(preds, 90, axis=0) # 90th percentile
            
            df_mean = pd.DataFrame(preds_mean, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
            df_min = pd.DataFrame(preds_min, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
            df_max = pd.DataFrame(preds_max, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
            return {"avg": df_mean, "worst": df_min, "best": df_max}
        else:
            preds_single = preds[0]
            pred_df = pd.DataFrame(preds_single, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
            return {"avg": pred_df, "worst": pred_df, "best": pred_df}
